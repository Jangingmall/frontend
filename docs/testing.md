...
`src/e2e/`에 8개 spec이 있다: `home.spec.ts`(홈), `products.spec.ts`(목록·필터),
`product-detail.spec.ts`(상세), `login.spec.ts`(로그인), `signup.spec.ts`(회원가입),
`checkout.spec.ts`(결제 폼 입력·무통장입금), `order-complete.spec.ts`(주문 완료),
`mypage-account.spec.ts`(마이페이지 회원정보). 결제는 토스페이먼츠 SDK 연동이 아직 없어
무통장입금 흐름만 검증한다 — "토스 모킹" 시나리오는 실제 SDK 연동 후 추가한다.

### 예비 시나리오 (남은 갭)

- 검색: 검색어 입력 → 결과
- 장바구니: 상세 → 담기 → 장바구니 확인 → 수량 변경, 게스트 → 로그인 병합, 장바구니 →
  결제 왕복(연결 전)
- 결제: 토스페이먼츠 SDK 연동 후 실제 위젯 흐름
- 마이페이지: 주문 목록·상세, 찜·최근 본 상품, 취소·교환·환불, 후기, 설정
...
| MVP E2E 시나리오 목록 | §6 구현된 시나리오 8개는 완료. 남은 예비 목록은 각 기능(카트 실연동, 토스 SDK 등) 완성과 함께 채운다                                                                  |
