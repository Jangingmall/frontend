...
백엔드는 도메인 변경 DB 트랜잭션 커밋 후, 재검증 이벤트와 최소 식별자를 전달한다. cache tag·FE 페이지 경로·화면 이름·Next.js 호출 정보는 요청에 넣지 않는다.
...
| 전송 시점    | 도메인 변경 트랜잭션 커밋 후 (트랜잭셔널 아웃박스) |
...
- 네트워크 오류·429·5xx → 지수 백오프 재시도.
- 400·401·403 → 반복 재시도하지 않고 요청 계약·시크릿·환경 설정 확인.
