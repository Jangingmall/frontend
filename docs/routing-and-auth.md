...
- `화면ID`는 팀 IA 시트 기준(최신본 확인일 2026-09-10)이다 — **단, 마이페이지(`/mypage/**`) 구간만 예외**: IA의 MY 번호 체계가 실제 Figma `[삼성가고싶어요] GUI` 파일과 어긋나는 걸 확인해(2026-09-16) 이 구간은 Figma 파일에 실제로 그려진 프레임 번호(`MY-1`부터 `MY-7`까지, `ID-1`부터 `ID-3`까지)를 화면ID로 쓴다(2026-09-20 정정). 이 표는 **라우트가 있는 화면만** 담는다. 모달·섹션·임베드형 화면(전역 헤더/푸터/플로팅, 상품 문의·후기 섹션, 옵션 변경·배송지 선택·PG 결제창·배송 조회·후기 작성·배송지 폼·결제수단 폼·비밀번호 변경 모달, **주문 취소·교환·반품 신청 모달**(마이페이지) 등)은 라우트가 없어 여기 없다.
...
| 마이페이지(루트)           | `/mypage`                                        | —                             | 로그인. **정정(2026-09-20)**: 옛 IA는 이 경로를 별도 대시보드(MY-1)로 가정했지만, 실제 Figma `[삼성가고싶어요] GUI` 파일엔 그런 대시보드 화면이 없다 — "마이페이지 진입 화면"은 바로 아래 `/mypage/orders`다. `/mypage/orders`로 **리다이렉트하지 않는다**(확정) — 이 경로는 별도 라우트로 유지하고, 지금은 placeholder("대시보드는 준비 중")만 있다. 실제로 무엇을 보여줄지는 여전히 미확정(§9) |
| 마이페이지 진입(주문·배송) | `/mypage/orders`                                 | MY-1                          | 로그인. 쿠폰함·가입일 미니 위젯 + 주문 상태 카운트 요약 + 기간·상태·장인 이름 필터 + 주문 리스트. 구현됨                                                                                                                                                                                                                                                                                         |
| 주문 상세                  | `/mypage/orders/[orderId]`                       | MY-1-OD                       | 로그인 + 본인 주문. 구현됨                                                                                                                                                                                                                                                                                                                                                                       |
| 취소·교환·환불 내역        | `/mypage/orders/cancellations`                   | MY-2                          | 로그인. `/mypage/orders`를 취소/교환·환불 상태로 필터링한 변형(같은 컴포넌트 재사용). 구현됨. 취소·교환·환불 **신청**은 별도 풀페이지가 아니라 모달(MY-request·MY-exchange)로 처리한다 — 아래 두 행(RT-1·RT-2, 옛 IA가 가정한 전용 라우트)은 폐기                                                                                                                                                |
| ~~주문 취소 신청~~         | ~~`/mypage/orders/[orderId]/cancel`~~            | ~~RT-1~~                      | **폐기** — Figma 실제 화면은 별도 라우트가 아니라 모달(MY-request)이다. 라우트 자체를 만들지 않는다                                                                                                                                                                                                                                                                                              |
...
| 마이페이지 루트(`/mypage`) 내용   | **부분 해결(2026-09-20)**: 옛 IA가 가정한 "대시보드(MY-1)"는 Figma 실제 화면에 없다 — 진짜 진입 화면은 `/mypage/orders`(Figma MY-1)다. `/mypage`는 그 화면으로 **redirect하지 않고 별도 라우트로 유지하는 것으로 확정**했다 — 다만 이 경로가 실제로 무엇을 보여줄지(빈 대시보드 자리를 계속 둘지, 다른 콘텐츠를 채울지)는 여전히 미확정. 적립금·구매등급 제도 자체도 여전히 미확정 | PM 확정 + 개발 편성                    |
| 결제수단 탭 실제 연동             | `/mypage/account?tab=payment-methods`은 구현됐으나 카드 목록·등록/수정 UI 없이 placeholder만 노출한다 — 카드번호·CVC를 직접 입력받는 Figma 시안 그대로 만들면 PCI-DSS 위반 소지가 있어(가맹점이 카드 원본정보를 취급할 수 없음) 실제로는 PG(토스페이먼츠) 빌링키 발급 위젯 연동이 필요하다                                                                                         | 빌링키 계약 확정 후 별도 작업으로 편성 |
| 내 정보(ID-1) 마케팅 수신동의     | Figma엔 "마케팅 수신동의" 행이 있으나 회원 API 응답(`GET /me`)에 그 값 자체가 없어 뺐다. **2026-09-21 정정**: 값 자체는 `GET\|PATCH /api/member/settings`(마이페이지 설정 화면, MY-7)로 조회·수정할 수 있다는 게 확인됐다 — BE 필드가 없는 게 아니라 `GET /me` 응답에만 없을 뿐이다. ID-1에 이 값을 다시 노출할지는 여전히 별도 판단(PM 확정) 필요                                 | PM 확정(노출 여부)                     |
| 취소·교환·환불 상태 전이 정책     | **부분 해결(2026-09-20)**: 취소/교환/환불 신청 화면 자체는 구현됨(모달, §2.1 MY-2 행) — 다만 "주문제작 착수 후 취소 불가" 같은 실제 상태 전이 허용 규칙은 여전히 정책 미확정이라 모든 상태에서 버튼을 노출하는 임시 로직으로 구현했다. 정책 확정 시 반영 필요                                                                                                                      | PM 확정                                |
| 회원 탈퇴 화면                    | MY-12(IA 번호, Figma 시안 자체가 없음)는 개발 백로그 미편성. 정책(90일 보관·거래기록 5년 보존)과 화면 설계 둘 다 미착수                                                                                                                                                                                                                                                            | PM 확정 + 개발 편성                    |
