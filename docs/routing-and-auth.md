...
| 마이페이지           | `/mypage`                                        | MY-1        | 로그인. 대시보드(프로필 + 최근 주문·찜·최근 본 상품) 계획이나 정책·개발 순번 미확정으로 지금은 placeholder만("준비 중" 안내, §9)                                                                                                                                           |
...
| 회원정보 수정        | `/mypage/account`                                | ID-1        | 로그인. `tab=info \| password \| addresses \| payment-methods`. 배송지·결제수단 편집은 모달. 좌측 내비의 "비밀번호 변경" 항목은 소셜 로그인 계정에서 숨긴다(비밀번호가 없는 계정에 죽은 탭을 두지 않으려는 제품 판단 — Figma 소셜 변형은 실제로는 이 항목을 숨기지 않는다) |
