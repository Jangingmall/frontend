...
| OAuth 로그인 시작 경로 불일치 | 이 문서와 [routing-and-auth.md](routing-and-auth.md) §4.1은 시작 경로를 `GET /api/member/oauth2/{provider}`로 적어뒀지만, 같은 문서 §7(Proxy)은 실제 Spring Security OAuth2 로그인 시작 경로를 `/oauth2/authorization/{provider}`(프레임워크 기본 경로, `/api` 밖)로 설명한다. 커스텀 REST 엔드포인트(`POST .../complete-profile`)와 프레임워크가 제공하는 리다이렉트 시작점은 다른 경로일 가능성이 높다 — 실제 도메인 연동 전에 BE 소스로 확정해야 한다 |
