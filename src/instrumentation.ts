 ...
 *
 * **`NODE_ENV === "production"`이면 플래그가 켜져 있어도 목업을 띄우지 않는다** —
 * 목업은 "어떤 mock 유저로든 로그인 가능"한 상태라(예: `admin@midam.test`만 알면 ADMIN
 * 신원 획득), 실수로 배포 환경 변수에 `NEXT_PUBLIC_API_MOCKING=enabled`가 남아 있어도
 * 외부에 노출되지 않도록 여기서 한 번 더 막는다(PR 리뷰 — CodeRabbit Security Review).
 */
